...
import { generateId, getMaxStringLengthFromList, getPathId, splitNameString } from '@/shared/utils/string'
...
export const getInitialState = (node, { x, y }, theme, type) => {
  ...
  const totalNamePartsNumber = nameParts.length
  const maxNamePartLength = getMaxStringLengthFromList(nameParts)
...
export const setupSharedPrint = state => ({
  ...
  printName(newPosition) {
    ...
    const { x, y } = newPosition || position
...
export const setupCompleteState = (initialState) => {
  const state = extractBasicState(initialState)
