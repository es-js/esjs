...
export async function activate(context: ExtensionContext) {
  const serverModule = context.asAbsolutePath(path.join('node_modules', 'vscode-languageserver', 'node.js'))
  ...
  client = new LanguageClient('esjs', 'EsJS Language Server', serverOptions, clientOptions)

  const keywordsProviders = vscode.languages.registerCompletionItemProvider('*', {
    provideCompletionItems() {
      return [
        ...Array.from(keywordControl).map(([esjsKeyword, jsKeyword]) => new vscode.CompletionItem(String(esjsKeyword))),
      ]
    },
  })
