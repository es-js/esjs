...
const esjsCompile = (source: string, filename: string) => compile(source, {
  compiler: 'essucrase',
});
...
export async function load(url: string, context: any, nextLoad: any) {
  if (extensionsRegex.test(url)) {
    const transformedSource = esjsCompile(await readFile(fileURLToPath(url), 'utf8'), fileURLToPath(url));
