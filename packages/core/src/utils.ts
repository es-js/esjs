...
export function splitScriptTemplate(code: string): SplitScriptTemplate {
  ...
  if (trimmedCode.startsWith(FRONT_MATTER_DIVISOR)) {
    script = trimmedCode.substring(trimmedCode.indexOf(FRONT_MATTER_DIVISOR) + 3)
    script = script.substring(0, script.lastIndexOf(FRONT_MATTER_DIVISOR)).trim()

    template = trimmedCode.substring(trimmedCode.lastIndexOf(FRONT_MATTER_DIVISOR) + 3).trim()
  }
