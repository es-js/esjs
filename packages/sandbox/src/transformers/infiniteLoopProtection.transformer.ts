...
export class InfiniteLoopProtectionTransformer implements Transformer {
  transform(code: string) {
    ...
    espree.parse(code, {
      ...
      this.addInfiniteLoopProtectionToBody(node, varStr, varPrefix, loopId, patches, checkStr)
    })
  ...
  addInfiniteLoopProtectionToBody (node: any, varStr: string, varPrefix: string, loopId: number, patches: {
    pos: number;
    str: string
  }[], checkStr: string) {
    switch (node.type) {
      case 'FunctionDeclaration':
        ...
        body.body.forEach((childNode: any) => {
          this.addInfiniteLoopProtectionToBody(childNode, varStr, varPrefix, loopId, patches, checkStr)
        })
      ...
      case 'ExportAllDeclaration':
        this.addInfiniteLoopProtectionToBody(node.declaration, varStr, varPrefix, loopId, patches, checkStr)
        break
